...
class NLI_FullLinear_3M(torch.nn.Module):

    def __init__(self, device='cuda'):
        super(NLI_FullLinear_3M, self).__init__()
        # input is batch_size x 6 x 1024
        self.l1 = torch.nn.Linear(1024*6, 1024)
        self.l2 = torch.nn.Linear(1024, 256)
        self.l3 = torch.nn.Linear(256, 64)
        self.l4 = torch.nn.Linear(64, 1)

        self.dropout = torch.nn.Dropout(0.1)
        self.lnorm = torch.nn.LayerNorm(1024)
...
# 0.70
class NLI_PairsBasic_3M(torch.nn.Module):

    def __init__(self, device='cuda'):
        super(NLI_PairsBasic_3M, self).__init__()
        # input is batch_size x 11 x 1024
        self.c1 = torch.nn.Linear(1024*2, 1024).to(device)
...
NUM_HEADS_1 = 4
DIMS_1 = 64
class NLI_Heads_3M(torch.nn.Module):
    
    class head(torch.nn.Module):
        def __init__(self):
            super(NLI_Heads_3M.head, self).__init__()
            # input is batch_size x 11 x 1024
    ...
    def __init__(self, device='cuda'):
        super(NLI_Heads_3M, self).__init__()
...
DIMS = 64
MINI_DIMS = 32
class NLI_MiniHeads_3M(torch.nn.Module):
    
    class head(torch.nn.Module):

        class mini_head(torch.nn.Module):
            def __init__(self):
                super(NLI_MiniHeads_3M.head.mini_head, self).__init__()
                # input is batch_size x 11 x 1024
        ...
        def __init__(self):
            super(NLI_MiniHeads_3M.head, self).__init__()
    ...
    def __init__(self, device='cuda'):
        super(NLI_MiniHeads_3M, self).__init__()
