...
def print_tracker(tracking, set_name, ma_ratio=None):
    ...
    average_enough_retrieved = np.array(tracking['average_total_retrieved'])
    ...
    plt.title('Accuracy and F1 Score for ' + set_name)
    ...
    plt.show()
    plt.figure(figsize=(10, 5))
    plt.title('Retrieval Metrics for ' + set_name)
